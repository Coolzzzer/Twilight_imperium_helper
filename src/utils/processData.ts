  ...
  data.forEach((element) => {
    ...
    element.set.forEach((el: any) => {
      ...
      if (isWin) {
        ...
        factionStats[fraction].winRate +=
          [0, 0.5, 0.66, 0.75, 0.8, 0.83, 0.86, 0.88][quantity - 1] ?? 1;
      } else {
        factionStats[fraction].losses++;
        factionStats[fraction].losers.push(` ${player}`);
        factionStats[fraction].winRate -=
          [0, 0.5, 0.33, 0.25, 0.2, 0.17, 0.14, 0.12][quantity - 1] ?? 1;
      }
    });
  });
  return factionStats;
...
export function generateFavoriteFactions(
  data: any[],
  ...
  for (const player in playerStats) {
    ...
    const eligible = Object.entries(factionGameStats).filter(
      ([_, stat]) => stat.games >= 2
    );
...
export function generateTierList(
  factionStats: Record<string, { winRate: number }>
) {
  const tierList: Record<string, string[]> = {
    S: [],
    A: [],
    B: [],
    C: [],
  ...
  for (const [fraction, data] of Object.entries(factionStats)) {
    ...
    else if (winRate <= -0.5 && win > 0) tierList.C.push(fraction);
    if (win == 0) tierList.O.push(fraction);
  }
  return tierList;
}
